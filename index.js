#!/usr/bin/env node
// Entry point for the WebGPU Inspector MCP server with shader debugging support.
//
// Runs:
//   - an MCP server over stdio (with 21 base inspection tools + 8 shader debugging tools)
...
import { Bridge } from "webgpu_inspector/claude-plugin/server/bridge.js";
import { BrowserController } from "webgpu_inspector/claude-plugin/server/browser.js";
...
async function main() {
  ...
  const shutdown = async () => {
    ...
    try {
      // Release port 9690 before exit so the next launch can bind it.
      await bridge.stop();
    } catch (e) {
      /* ignore */
    }
